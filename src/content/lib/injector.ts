...
export function createInjector(
  ...
  function instrumentBlocks(descs: BlockDescriptor[]): void {
    ...
    for (const desc of descs) {
      ...
      if (existing) {
        if (existing.blockEl.isConnected) continue
        existing.blockEl.querySelectorAll('[data-thr-trigger]').forEach(b => b.remove())
        delete existing.blockEl.dataset.thrId
        delete existing.blockEl.dataset.thrState
